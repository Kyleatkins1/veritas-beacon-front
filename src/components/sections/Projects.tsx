  ...
  return <section id="projects" className="section-padding bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {projects.map((project, index) => <div key={index} className={`bg-gray-50 rounded-xl p-6 border border-gray-100 transition-all hover:shadow-md relative overflow-hidden`}>
              {index === 0 && <span className="absolute left-6 top-4 bg-green-500 text-white text-xs font-semibold px-3 py-1 rounded-full z-10 animate-fade-in">LIVE</span>}
              <div className="flex flex-col h-full">
                ...
                <div className="mt-auto flex flex-col gap-3">
                  {index === 0 ? <a href={project.link} target="_blank" rel="noopener noreferrer" className="inline-block">
                      ...
                      <Button variant="outline" className="border-veritas-primary/30 text-veritas-primary hover:bg-veritas-primary/5" onClick={() => setSelectedProject(index)}>
                        Learn more
                      </Button>
                    </>}
                  {index === 0 && <Button variant="outline" className="border-green-300 text-green-600 hover:bg-green-50 w-full" onClick={() => setSelectedProject(index)}>
                      Learn more
                    </Button>}
                </div>
              </div>
      ...
      <Dialog open={selectedProject !== null} onOpenChange={() => setSelectedProject(null)}>
        <DialogContent className="sm:max-w-[800px]">
          ...
          {selectedProject !== null && selectedProject > 3 && <>
              ...
              <div className="mt-4">
                {projects[selectedProject].previewImage && <img src={projects[selectedProject].previewImage} alt={`${projects[selectedProject].title} Dashboard`} className="w-full max-h-[500px] object-contain rounded-md shadow-md mb-4" />}
                <div className="space-y-2">
